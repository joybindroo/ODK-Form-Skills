...
3. **pyMCP (Database Analysis Engine)**
   ...
   - *Installation*: 
     - Clone the repo: `git clone https://github.com/joybindroo/pyMCP.git`
     - Install: `cd pyMCP && pip install -r requirements.txt`
     - Configuration: In the server configuration, set the database path to the SQLite file generated by `pyODKmcp` (e.g., `sqlite://path/to/odk_mcp_server.db`).
     - Activation: Register the server in your MCP client (Claude Desktop, VS Code, etc.) to enable SQL tool calls.
